...
if __name__ == '__main__':#只有用到训练集和测试集的编号才需要-1
        ...
        for i in range(len(groups)):
            for j in range(1, i_num):
                if len(participant_list[j]) > 300:
                    cand_ratings[i][j] = cand_ratings[i][j] / math.log(len(participant_list[j]), 350)
    top_n = int(top_n)
    recommend_list = []
    for i in range(len(groups)):
    ...
    for i in range(len(groups)):
        for j in range(len(recommend_list[i])):
            if recommend_list[i][j] not in items:
                items.append(recommend_list[i][j])
    ...
    Novelty = Novelty / top_n / len(groups)
    Precision = hits/Precision
    Recall = hits/Recall
    f1 = 2*Precision*Recall/(Recall+Precision)
    NDCG = DCG/(iDCG * len(groups))
    Coverage = len(items)/i_num
